        ...
        return(
            <>
                <Col span={8} style={{margin: '0 auto'}}>
                    ...
                    <Pagination defaultCurrent={curentPage} defaultPageSize={perPage} total={todoList.length} showSizeChanger={false} showTotal={(total, range) => `${range[0]}-${range[1]} of ${total} items`} showQuickJumper={true} size="small" onChange={(page, pageSize) => this.handlerChangePagination(page, pageSize)}/>
                </Col>
            </>
        )
    }
